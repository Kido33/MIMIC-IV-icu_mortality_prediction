...
class MultiModalMIMIC_Focal(nn.Module):
    def __init__(self, time_dim=22, static_dim=48, hidden_dim=64):
        ...
        # [수정] 에러 메시지에 따라 입력 차원을 112가 아닌 64로 설정
        # 저장된 가중치가 [1, 64]이므로 LSTM 출력만 사용함을 의미합니다.
        self.fc = nn.Linear(hidden_dim, 1) 
        self.sigmoid = nn.Sigmoid()

    def forward(self, x_time, x_static):
        # x_static은 입력받지만, 저장된 모델 구조에 따라 연산에서는 제외합니다.
        _, (h_n, _) = self.lstm(x_time)
        # h_n[-1]의 shape: (batch, 64)
...
def run_comprehensive_benchmark(disease_type):
    print(f"\n🔥 [{disease_type.upper()}] 8종 벤치마킹 및 Calibration 통합 분석")
    ...
    # 1. Ours (MultiModal)
    model_ours = MultiModalMIMIC_Focal(static_dim=48, hidden_dim=64).to(device)
    model_path = f"{BASE_DIR}/{disease_type}_best_model_v3.pth"
    ...
    for name, model in tqdm(ml_models.items(), desc=" ⚡ ML Training"):
    ...
    for name, model in dl_models.items():
        ...
        for _ in range(3):
            for bx, by in train_loader:
                ...
                optimizer.zero_grad(); nn.BCELoss()(model(bx), by).backward(); optimizer.step()
    ...
    plt.title(f"Calibration: {disease_type.upper()}")
    plt.xlabel("Predicted Prob"); plt.ylabel("Actual Prob"); plt.legend(); plt.grid(True)
    
    png_path = f"{BASE_DIR}/{disease_type}_calibration_final.png"
    plt.savefig(png_path, dpi=300); plt.close()
    
    df_res = pd.DataFrame(results).sort_values('AUROC', ascending=False)
    csv_path = f"{BASE_DIR}/{disease_type}_benchmark_final.csv"
    df_res.to_csv(csv_path, index=False)
    
    print(f"\n📊 [{disease_type.upper()}] 결과 요약\n", df_res.to_string(index=False))
    print(f"✅ 저장 완료: {csv_path}, {png_path}")
