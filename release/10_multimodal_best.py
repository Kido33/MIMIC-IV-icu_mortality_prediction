...
# ==========================================
# [1. 모델 정의 구역] - 체크포인트 규격(48차원)에 맞춤
# ==========================================

class MultiModalMIMIC_Focal(nn.Module):
    def __init__(self, time_dim=22, static_dim=48, hidden_dim=64): # static_dim=48로 고정
        super().__init__()
        self.lstm = nn.LSTM(time_dim, hidden_dim, num_layers=2, batch_first=True, dropout=0.2)
        # 체크포인트 규격인 [32, 48]에 맞춰 Linear(48, 32)로 설정
...
def run_comprehensive_benchmark(disease_type):
    ...
    # --- 1. Ours (MultiModal) - static_dim=48로 호출 ---
    model_ours = MultiModalMIMIC_Focal(static_dim=48, hidden_dim=64).to(device)
    model_path = f"{BASE_DIR}/{disease_type}_multimodal_best.pth"
    ...
    for name, model in dl_models.items():
        ...
        model.train()
        for _ in range(3): # 빠른 분석을 위해 3 Epoch
    ...
    png_path = f"{BASE_DIR}/{disease_type}_calibration_smote.png"
    plt.savefig(png_path, dpi=300); plt.close()
    
    df_res = pd.DataFrame(results).sort_values('AUROC', ascending=False)
    csv_path = f"{BASE_DIR}/{disease_type}_benchmark_smote.csv"
